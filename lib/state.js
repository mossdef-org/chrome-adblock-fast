...
async function updateBadge(status, state) {
	try {
		if (state === STATES.PAUSED) {
			await chrome.action.setBadgeText({ text: '' });
		} else if (status && status.entries) {
...
function hostPermissionOrigin(routerUrl) {
	try {
		const u = new URL(routerUrl);
		return u.origin + '/ubus';
	} catch (e) {
		return null;
	}
}

async function requestHostPermission(routerUrl) {
	const origin = hostPermissionOrigin(routerUrl);
	if (!origin) return false;
	return chrome.permissions.request({ origins: [origin] });
}

async function hasHostPermission(routerUrl) {
	const origin = hostPermissionOrigin(routerUrl);
	if (!origin) return false;
